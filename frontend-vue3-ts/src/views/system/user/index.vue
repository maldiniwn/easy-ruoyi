<template>
  <div class="app-container">
    ...
    <el-form :model="reactiveData.queryParams" ref="queryFormRef" v-show="showSearch" :inline="true" label-width="70px">
      <el-form-item label="用户名">
        <el-input v-model="reactiveData.queryParams.userName" placeholder="请输入用户名" clearable style="width: 240px" />
      </el-form-item>
      <el-form-item label="姓名">
        <el-input v-model="reactiveData.queryParams.name" placeholder="请输入姓名" clearable style="width: 240px" />
      </el-form-item>
      <el-form-item label="是否启用">
        <el-select v-model="reactiveData.queryParams.isEnable" placeholder="请选择是否启用" clearable style="width: 240px">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="danger" plain icon="delete" size="small" :loading="exportLoading" :disabled="multiple"
          @click="handleBatchDelete" v-hasPermi="['system:user:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table stripe border ref="pageTableRef" v-loading="loading" :data="reactiveData.data.rows"
      ...
      <el-table-column label="性别" prop="enumGenderStr" min-width="60" show-overflow-tooltip />
      ...
      <el-table-column label="手机号" prop="tel" min-width="120" show-overflow-tooltip />

      <el-table-column label="身份证" prop="sid" min-width="150" show-overflow-tooltip />

      <el-table-column label="出生日期" prop="bornDateStr" min-width="150" show-overflow-tooltip />
      ...
      <el-table-column label="操作" align="center" min-width="230" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <template v-if="scope.row.id != proxy.$superAdmin">
            <el-link class="el-link-spacing" :underline="false" size="small" type="primary" icon="edit"
              @click="handleUpdate(scope.row)" v-hasPermi="['system:user:edit']"><span
  ...
  <userGrantForm ref="userGrantFormRef"></userGrantForm>
</template>
...
import { pageUser, addUser, editUser, delUser, batchdelUser, exportUser } from "@/api/system/user"
import detailForm from "./detailForm.vue"
import addForm from "./addForm.vue"
import editForm from "./editForm.vue"
import importForm from "./importForm.vue"
import userGrantForm from "./userGrantForm.vue"
...
const resetSearch = () => {
  ...
  reactiveData.queryParams.userName = "";
...
const handleRefresh = () => {
  loading.value = true;
  pageUser(reactiveData.queryParams).then((resp: any) => {
...
const handleGrant = (row: any) => {
  userGrantFormRef.value?.show(row.id);
}
  ...
  if (ids.length == 0) {
    ...
    ElMessageBox.confirm("确定删除选中的角色吗？",
      "Warning",
      {
        ...
        let data = { ids };
        batchdelUser(data).then((resp: any) => {
          if (resp.code == 200 && resp.success) {
            ElMessage({
              type: "success",
              message: "删除成功！"
...
//导出
const handleExport = () => {
  exportUser(reactiveData.queryParams);
}
